<template>
  <div>
    <ul class="icon-list">
      <li v-for="(icon,index) in icons" :key="index" :class="{active: currentIndex === index && isShow}" @click="chooseIconBtn(icon,index)">
        <i :class="[icon.class]" style="font-size:40px;" />
...
export default {
  ...
  methods: {
    chooseIconBtn(icon, index) {
      this.$emit('handleChange', icon) // 触发父组件中handleChange事件并传参icon
